...
// ----------------------------------------------------------------------------
// PreferencesInstance.cpp - Released 2025-04-07T08:53:56Z
// ----------------------------------------------------------------------------
...
void PreferencesInstance::Assign( const ProcessImplementation& p )
{
   ...
   if ( x != nullptr )
   {
      ...
      /*
       * Enforce system-dependent limits.
       */
      int numProcessors = Module->NumberOfProcessors();
      process.maxProcessors = pcl::Min( process.maxProcessors, numProcessors );
      process.maxFileReadThreads = pcl::Min( process.maxFileReadThreads, numProcessors );
      process.maxFileWriteThreads = pcl::Min( process.maxFileWriteThreads, numProcessors );
   }
}
...
void* PreferencesInstance::LockParameter( const MetaParameter* p, size_type tableRow )
{
   ...
   if ( p == METAPARAMETER_INSTANCE_ID( Process, initCUDARuntimeAtStartup ) )
      return &process.initCUDARuntimeAtStartup;
...
// ----------------------------------------------------------------------------
// EOF PreferencesInstance.cpp - Released 2025-04-07T08:53:56Z
