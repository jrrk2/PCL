...
// ----------------------------------------------------------------------------
// pcl/UIObject.cpp - Released 2025-04-07T08:53:32Z
// ----------------------------------------------------------------------------
...
#ifdef __PCL_DEBUG_UIOBJECT_INDEX
      API_UI_SetHandleDestroyedEventRoutine( object->handle, UIEventDispatcher::HandleDestroyed );
#endif
   }
...
UIObject::UIObject( const void* h )
   : handle( const_cast<void*>( h ) )
{
   if ( handle != nullptr )
      s_objects.Add( this );
}
...
UIObject::UIObject( const UIObject& x )
   : handle( x.handle )
{
   if ( handle != nullptr )
   {
      if ( API_UI_AttachToUIObject( ModuleHandle(), handle ) == api_false )
      {
         handle = nullptr;
...
UIObject::~UIObject() noexcept( false )
{
   if ( handle != nullptr )
   {
      ...
      if ( API_UI_GetUIObjectRefCount( theHandle ) > 0 )
         if ( API_UI_DetachFromUIObject( ModuleHandle(), theHandle ) == api_false )
...
{
   return (handle != nullptr) ? API_UI_GetUIObjectRefCount( handle ) : size_type( 0 );
}
...
IsoString UIObject::ObjectType() const
{
   ...
   size_type len = 0;
   API_UI_GetUIObjectType( handle, nullptr, &len );
   if ( len == 0 )
      throw APIFunctionError( "GetUIObjectType" );

   IsoString objType;
   objType.SetLength( len );
   if ( API_UI_GetUIObjectType( handle, objType.Begin(), &len ) == api_false )
      throw APIFunctionError( "GetUIObjectType" );
...
String UIObject::ObjectId() const
{
   ...
   size_type len = 0;
   API_UI_GetUIObjectId( handle, nullptr, &len );

   String objId;
   if ( len > 0 )
   {
      objId.SetLength( len );
      if ( API_UI_GetUIObjectId( handle, objId.Begin(), &len ) == api_false )
...
{
   if ( API_UI_SetUIObjectId( handle, id.c_str() ) == api_false )
...
void UIObject::SetHandle( void* newHandle )
{
   if ( newHandle != handle )
   {
      if ( handle != nullptr )
      {
         ...
         if ( API_UI_GetUIObjectRefCount( oldHandle ) > 0 )
            if ( API_UI_DetachFromUIObject( ModuleHandle(), oldHandle ) == api_false )
               throw APIFunctionError( "DetachFromUIObject" );
      }

      if ( newHandle != nullptr )
      {
         if ( API_UI_AttachToUIObject( ModuleHandle(), newHandle ) == api_false )
            throw APIFunctionError( "AttachToUIObject" );
...
// ----------------------------------------------------------------------------
// EOF pcl/UIObject.cpp - Released 2025-04-07T08:53:32Z
