...
ToolButton::ToolButton( const String& text, const pcl::Bitmap& icon, bool checkable, Control& parent )
   : Button( API_Button_CreateToolButton( ModuleHandle(), this, text.c_str(), icon.handle, checkable, parent.handle, 0/*flags*/ ) )
{
   if ( handle == 0 )
...
{
   return API_Button_GetToolButtonCheckable( handle ) != api_false;
}
...
{
   API_Button_SetToolButtonCheckable( handle, checkable );
}
