...
// ----------------------------------------------------------------------------
// pcl/ProcessParameter.cpp - Released 2025-04-07T08:53:32Z
// ----------------------------------------------------------------------------
...
public:
   ...
   ProcessParameterPrivate( meta_parameter_handle hParam )
   {
      if ( hParam != nullptr )
      {
         meta_process_handle hProcess = API_Process_GetParameterProcess( hParam );
         if ( hProcess == nullptr )
...
{
   m_data = new ProcessParameterPrivate( API_Process_GetParameterByName( process.Handle(), paramId.c_str() ) );
...
{
   m_data = new ProcessParameterPrivate( API_Process_GetTableColumnByName( table.m_data->handle, colId.c_str() ) );
   if ( m_data->handle == nullptr )
   {
      if ( table.IsNull() )
...
ProcessParameter::~ProcessParameter()
{
   ...
   // because ProcessParameterPrivate is not defined.
}
...
{
   return ProcessParameter( API_Process_GetParameterTable( m_data->handle ) );
}
...
{
   size_type len = 0;
   API_Process_GetParameterIdentifier( m_data->handle, 0, &len );

   IsoString id;
   if ( len > 0 )
   {
      id.SetLength( len );
      if ( API_Process_GetParameterIdentifier( m_data->handle, id.Begin(), &len ) == api_false )
...
{
   size_type len = 0;
   API_Process_GetParameterAliasIdentifiers( m_data->handle, 0, &len );
   ...
   if ( len > 0 )
   {
      ...
      if ( API_Process_GetParameterAliasIdentifiers( m_data->handle, csAliases.Begin(), &len ) == api_false )
...
{
   return API_Process_GetParameterRequired( m_data->handle ) != api_false;
}
...
{
   return API_Process_GetParameterReadOnly( m_data->handle ) != api_false;
}
...
{
   size_type len = 0;
   API_Process_GetParameterDescription( m_data->handle, 0, &len );

   String description;
   if ( len > 0 )
   {
      description.SetLength( len );
      if ( API_Process_GetParameterDescription( m_data->handle, description.Begin(), &len ) == api_false )
...
{
   size_type len = 0;
   API_Process_GetParameterScriptComment( m_data->handle, 0, &len );

   String comment;
   if ( len > 0 )
   {
      comment.SetLength( len );
      if ( API_Process_GetParameterScriptComment( m_data->handle, comment.Begin(), &len ) == api_false )
...
ProcessParameter::data_type ProcessParameter::Type() const
{
   ...
   uint32 apiType = API_Process_GetParameterType( m_data->handle );
   if ( apiType == 0 )
...
Variant ProcessParameter::DefaultValue() const
{
   ...
   {
      size_type len = 0;
      API_Process_GetParameterDefaultValue( m_data->handle, 0, &len );

      String value;
      if ( len > 0 )
      {
         value.SetLength( len );
         if ( API_Process_GetParameterDefaultValue( m_data->handle, value.Begin(), &len ) == api_false )
   ...
   {
      int index = API_Process_GetParameterDefaultElementIndex( m_data->handle );
      if ( index < 0 )
   ...
   if ( API_Process_GetParameterDefaultValue( m_data->handle, &value, 0 ) == api_false )
      throw APIFunctionError( "GetParameterDefaultValue" );
...
void ProcessParameter::GetNumericRange( double& minValue, double& maxValue ) const
{
   ...
   if ( API_Process_GetParameterRange( m_data->handle, &minValue, &maxValue ) == api_false )
...
int ProcessParameter::Precision() const
{
   if ( !IsReal() )
      return 0;
   return API_Process_GetParameterPrecision( m_data->handle );
}
...
bool ProcessParameter::ScientificNotation() const
{
   if ( !IsReal() )
      return false;
   return API_Process_GetParameterScientificNotation( m_data->handle ) != api_false;
}
...
void ProcessParameter::GetLengthLimits( size_type& minLength, size_type& maxLength ) const
{
   ...
   if ( API_Process_GetParameterLengthLimits( m_data->handle, &minLength, &maxLength ) == api_false )
...
ProcessParameter::enumeration_element_list ProcessParameter::EnumerationElements() const
{
   ...
   size_type count = API_Process_GetParameterElementCount( m_data->handle );
   if ( count == 0 )
   ...
   {
      size_type len = 0;
      API_Process_GetParameterElementIdentifier( m_data->handle, i, 0, &len );
      if ( len == 0 )
         throw APIFunctionError( "GetParameterElementIdentifier" );
      elements[i].id.SetLength( len );
      if ( API_Process_GetParameterElementIdentifier( m_data->handle, i, elements[i].id.Begin(), &len ) == api_false )
         throw APIFunctionError( "GetParameterElementIdentifier" );
      elements[i].id.ResizeToNullTerminated();

      len = 0;
      API_Process_GetParameterElementAliasIdentifiers( m_data->handle, i, 0, &len );
      if ( len > 0 )
      {
         IsoString aliases;
         aliases.SetLength( len );
         if ( API_Process_GetParameterElementAliasIdentifiers( m_data->handle, i, aliases.Begin(), &len ) == api_false )
            throw APIFunctionError( "GetParameterElementAliasIdentifiers" );
         aliases.ResizeToNullTerminated();
         aliases.Break( elements[i].aliases, ',' );
      }

      elements[i].value = API_Process_GetParameterElementValue( m_data->handle, i );
   }
...
String ProcessParameter::AllowedCharacters() const
{
   ...
   {
      size_type len = 0;
      API_Process_GetParameterAllowedCharacters( m_data->handle, 0, &len );

      if ( len > 0 )
      {
         allowed.SetLength( len );
         if ( API_Process_GetParameterAllowedCharacters( m_data->handle, allowed.Begin(), &len ) == api_false )
...
ProcessParameter::parameter_list ProcessParameter::TableColumns() const
{
   ...
   if ( IsTable() )
      if ( API_Process_EnumerateTableColumns( m_data->handle,
...
// ----------------------------------------------------------------------------
// EOF pcl/ProcessParameter.cpp - Released 2025-04-07T08:53:32Z
