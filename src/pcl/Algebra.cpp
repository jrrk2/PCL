...
void PCL_FUNC InPlaceGaussJordan( Matrix& A, Matrix& B )
{
   A.EnsureUnique();
   B.EnsureUnique();
   if ( API_Numerical_GaussJordanInPlaceD( A.DataPtr(), B.DataPtr(), A.Rows(), B.Columns() ) == api_false )
...
void PCL_FUNC InPlaceGaussJordan( FMatrix& A, FMatrix& B )
{
   A.EnsureUnique();
   B.EnsureUnique();
   if ( API_Numerical_GaussJordanInPlaceF( A.DataPtr(), B.DataPtr(), A.Rows(), B.Columns() ) == api_false )
...
void PCL_FUNC InPlaceSVDImplementation( Matrix& A, Vector& W, Matrix& V )
{
   ...
   W = Vector( n );
   V = Matrix( n, n );
   if ( API_Numerical_SVDInPlaceD( A.DataPtr(), W.DataPtr(), V.DataPtr(), m, n ) == api_false )
...
void PCL_FUNC InPlaceSVDImplementation( FMatrix& A, FVector& W, FMatrix& V )
{
   ...
   W = FVector( n );
   V = FMatrix( n, n );
   if ( API_Numerical_SVDInPlaceF( A.DataPtr(), W.DataPtr(), V.DataPtr(), m, n ) == api_false )
