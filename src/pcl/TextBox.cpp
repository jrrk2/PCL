...
TextBox::TextBox( const String& text, Control& parent )
   : Frame( API_TextBox_CreateTextBox( ModuleHandle(), this, text.c_str(), parent.handle, 0/*flags*/ ) )
{
   if ( IsNull() )
...
{
   size_type len = 0;
   API_TextBox_GetTextBoxText( handle, 0, &len );

   String text;
   if ( len > 0 )
   {
      text.SetLength( len );
      if ( API_TextBox_GetTextBoxText( handle, text.Begin(), &len ) == api_false )
...
{
   API_TextBox_SetTextBoxText( handle, text.c_str() );
}
...
{
   return API_TextBox_GetTextBoxReadOnly( handle ) != api_false;
}
...
{
   API_TextBox_SetTextBoxReadOnly( handle, readOnly );
}
...
{
   API_TextBox_SetTextBoxSelected( handle, select );
}
...
{
   selStart = selEnd = 0;
   API_TextBox_GetTextBoxSelection( handle, &selStart, &selEnd );
}
...
{
   API_TextBox_SetTextBoxSelection( handle, selStart, selEnd );
}
...
{
   size_type len = 0;
   API_TextBox_GetTextBoxSelectedText( handle, 0, &len );

   String text;
   if ( len > 0 )
   {
      text.SetLength( len );
      if ( API_TextBox_GetTextBoxSelectedText( handle, text.Begin(), &len ) == api_false )
...
{
   return API_TextBox_GetTextBoxCaretPosition( handle );
}
...
{
   API_TextBox_SetTextBoxCaretPosition( handle, pos );
}
...
{
   API_TextBox_InsertTextBoxText( handle, text.c_str() );
}
...
void TextBox::Delete()
{
   API_TextBox_DeleteTextBoxText( handle );
}
...
{
   INIT_EVENT_HANDLERS();
   if ( API_TextBox_SetTextBoxUpdatedEventRoutine( handle, &receiver,
...
{
   INIT_EVENT_HANDLERS();
   if ( API_TextBox_SetTextBoxCaretPositionUpdatedEventRoutine( handle, &receiver,
...
{
   INIT_EVENT_HANDLERS();
   if ( API_TextBox_SetTextBoxSelectionUpdatedEventRoutine( handle, &receiver,
