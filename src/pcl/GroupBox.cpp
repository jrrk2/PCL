...
GroupBox::GroupBox( const String& title, Control& parent )
   : Control( API_GroupBox_CreateGroupBox( ModuleHandle(), this, title.c_str(), parent.handle, 0/*flags*/ ) )
{
   if ( IsNull() )
...
{
   size_type len = 0;
   API_GroupBox_GetGroupBoxTitle( handle, 0, &len );

   String title;
   if ( len > 0 )
   {
      title.SetLength( len );
      if ( API_GroupBox_GetGroupBoxTitle( handle, title.Begin(), &len ) == api_false )
...
{
   API_GroupBox_SetGroupBoxTitle( handle, title.c_str() );
}
...
{
   return API_GroupBox_GetGroupBoxCheckable( handle ) != api_false;
}
...
{
   API_GroupBox_SetGroupBoxCheckable( handle, enabled );
}
...
{
   return API_GroupBox_GetGroupBoxChecked( handle ) != api_false;
}
...
{
   API_GroupBox_SetGroupBoxChecked( handle, checked );
}
...
{
   INIT_EVENT_HANDLERS();
   if ( API_GroupBox_SetGroupBoxCheckEventRoutine( handle, &receiver,
