...
Timer::Timer()
   : UIObject( API_Timer_CreateTimer( ModuleHandle(), this, 0/*flags*/ ) )
{
   if ( handle == 0 )
...
void Timer::OnTimer( timer_event_handler f, Control& receiver )
{
   onTimer = nullptr;
   count = 0;
   if ( API_Timer_SetTimerNotifyEventRoutine( handle, &receiver,
...
{
   uint32 msec = 0;
   API_Timer_GetTimerInterval( handle, &msec );
...
{
   count = 0;
   API_Timer_SetTimerInterval( handle, (uint32)(1000*Round( sec, 3 )) );
}
...
{
   return API_Timer_GetTimerSingleShot( handle ) != api_false;
}
...
{
   count = 0;
   API_Timer_SetTimerSingleShot( handle, singleShot );
}
...
{
   return API_Timer_IsTimerActive( handle ) != api_false;
}
...
{
   count = 0;
   if ( API_Timer_StartTimer( handle ) == api_false )
