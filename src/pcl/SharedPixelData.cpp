...
// ----------------------------------------------------------------------------
// pcl/SharedPixelData.cpp - Released 2025-04-07T08:53:32Z
// ----------------------------------------------------------------------------
   ...
   : m_handle( handle )
{
   ...
   uint32 n;
   api_bool f;
   if ( API_SharedImage_GetImageFormat( m_handle, &n, &f ) == api_false )
   ...
   if ( API_SharedImage_AttachToImage( m_handle, this/*owner*/ ) == api_false )
...
SharedPixelData::SharedPixelData( int width, int height, int numberOfChannels,
   ...
   : m_handle( API_SharedImage_CreateImage( width, height, numberOfChannels,
...
bool SharedPixelData::IsAliased() const
{
   ...
   uint32 n = 0;
   if ( API_SharedImage_GetImageRefCount( m_handle, &n ) == api_false )
...
{
   return m_handle != nullptr && API_SharedImage_GetImageOwner( m_handle ) == this;
}
...
{
   if ( m_handle != nullptr )
      if ( API_SharedImage_AttachToImage( m_handle, this ) == api_false )
         throw APIFunctionError( "AttachToImage" );
}
...
{
   if ( m_handle != nullptr )
      if ( API_SharedImage_DetachFromImage( m_handle, this ) == api_false )
...
void* SharedPixelData::Allocate( size_type size ) const
{
   if ( size > 0 )
   {
      ...
      void* p = (m_handle == nullptr) ? PCL_ALIGNED_MALLOC( size, 32 ) : API_Global_Allocate( size );
...
void SharedPixelData::Deallocate( void* p ) const
{
   if ( p != nullptr )
      if ( m_handle == nullptr )
         PCL_ALIGNED_FREE( p );
      else if ( API_Global_Deallocate( p ) == api_false )
...
void** SharedPixelData::GetSharedData() const
{
   ...
   void** data;
   if ( API_SharedImage_GetImagePixelData( m_handle, &data ) == api_false )
...
{
   if ( m_handle != nullptr )
      if ( API_SharedImage_SetImagePixelData( m_handle, ptrToShared ) == api_false )
...
void SharedPixelData::GetSharedGeometry( int& width, int& height, int& numberOfChannels )
{
   ...
   {
      uint32 w, h, n;
      if ( API_SharedImage_GetImageGeometry( m_handle, &w, &h, &n ) == api_false )
...
{
   if ( m_handle != nullptr )
      if ( API_SharedImage_SetImageGeometry( m_handle, width, height, numberOfChannels ) == api_false )
...
void SharedPixelData::GetSharedColor( color_space& colorSpace, RGBColorSystem& RGBWS )
{
   ...
   {
      uint32 cs;
      if ( API_SharedImage_GetImageColorSpace( m_handle, &cs ) == api_false )
         throw APIFunctionError( "GetImageColorSpace" );

      api_RGBWS rgbws;
      if ( API_SharedImage_GetImageRGBWS( m_handle, &rgbws ) == api_false )
...
void SharedPixelData::SetSharedColor( color_space colorSpace, const RGBColorSystem&/*RGBWS*/ )
{
   if ( m_handle != nullptr )
   {
      if ( API_SharedImage_SetImageColorSpace( m_handle, colorSpace ) == api_false )
...
// ----------------------------------------------------------------------------
// EOF pcl/SharedPixelData.cpp - Released 2025-04-07T08:53:32Z
