...
ScrollBox::ScrollBox( Control& parent )
   : Frame( API_ScrollBox_CreateScrollBox( ModuleHandle(), this, parent.handle, 0/*flags*/ ) )
   , m_viewport( nullptr )
{
   if ( IsNull() )
      throw APIFunctionError( "CreateScrollBox" );

   m_viewport.TransferHandle( API_ScrollBox_CreateScrollBoxViewport( handle, &m_viewport ) );
...
{
   api_bool h, v;
   API_ScrollBox_GetScrollBarsVisible( handle, &h, &v );
   return h != api_false;
}
...
{
   API_ScrollBox_SetScrollBarsVisible( handle, showHorz, showVert );
}
...
{
   api_bool h, v;
   API_ScrollBox_GetScrollBoxAutoScrollEnabled( handle, &h, &v );
   return h != api_false;
}
...
{
   API_ScrollBox_SetScrollBoxAutoScrollEnabled( handle, enableHorz, enableVert );
}
...
{
   API_ScrollBox_GetScrollBoxHorizontalRange( handle, &minPos, &maxPos );
}
...
{
   API_ScrollBox_GetScrollBoxVerticalRange( handle, &minPos, &maxPos );
}
...
{
   API_ScrollBox_SetScrollBoxHorizontalRange( handle, minPos, maxPos );
}
...
{
   API_ScrollBox_SetScrollBoxVerticalRange( handle, minPos, maxPos );
}
...
{
   int32 h, v;
   API_ScrollBox_GetScrollBoxPageSize( handle, &h, &v );
   return h;
}
...
{
   API_ScrollBox_SetScrollBoxPageSize( handle, width, height );
}
...
{
   int32 h, v;
   API_ScrollBox_GetScrollBoxLineSize( handle, &h, &v );
   return h;
}
...
{
   API_ScrollBox_SetScrollBoxLineSize( handle, width, height );
}
...
{
   pcl::Point p;
   API_ScrollBox_GetScrollBoxPosition( handle, &p.x, &p.y );
   return p;
}
...
{
   API_ScrollBox_SetScrollBoxPosition( handle, horzPos, vertPos );
}
...
{
   api_bool h, v;
   API_ScrollBox_GetScrollBoxTrackingEnabled( handle, &h, &v );
   return h != api_false;
}
...
{
   API_ScrollBox_SetScrollBoxTrackingEnabled( handle, enableHorz, enableVert );
}
...
{
   INIT_EVENT_HANDLERS();
   if ( API_ScrollBox_SetScrollBoxHorizontalPosUpdatedEventRoutine( handle, &receiver,
...
{
   INIT_EVENT_HANDLERS();
   if ( API_ScrollBox_SetScrollBoxVerticalPosUpdatedEventRoutine( handle, &receiver,
...
{
   INIT_EVENT_HANDLERS();
   if ( API_ScrollBox_SetScrollBoxHorizontalRangeUpdatedEventRoutine( handle, &receiver,
...
{
   INIT_EVENT_HANDLERS();
   if ( API_ScrollBox_SetScrollBoxVerticalRangeUpdatedEventRoutine( handle, &receiver,
