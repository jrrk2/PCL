...
// ----------------------------------------------------------------------------
// pcl/ICCProfile.cpp - Released 2025-04-07T08:53:32Z
// ----------------------------------------------------------------------------
...
LCMSErrorMessage::LCMSErrorMessage()
   : thread( API_Thread_GetCurrentThread() )
{
}
...
StringList ICCProfile::ProfileDirectories()
{
   ...
   {
      size_type len = 0;
      API_Global_GetProfilesDirectory( i, 0, &len );
      if ( len == 0 )
         break;

      String path;
      path.SetLength( len );
      if ( API_Global_GetProfilesDirectory( i, path.Begin(), &len ) == api_false )
...
// ----------------------------------------------------------------------------
// EOF pcl/ICCProfile.cpp - Released 2025-04-07T08:53:32Z
