...
bool PixInsightSettings::GlobalFlag( const IsoString& globalId )
{
   ...
   (void)API_Global_GetGlobalFlag( globalId.c_str(), &b );
   return b != api_false;
}
...
{
   int32 i = 0;
   (void)API_Global_GetGlobalInteger( globalId.c_str(), &i, api_true );
   return i;
}
...
{
   uint32 u = 0;
   (void)API_Global_GetGlobalInteger( globalId.c_str(), &u, api_false );
   return u;
}
...
{
   double r = 0.0;
   (void)API_Global_GetGlobalReal( globalId.c_str(), &r );
   return r;
}
...
{
   uint32 rgba = 0;
   (void)API_Global_GetGlobalColor( globalId.c_str(), &rgba );
   return rgba;
}
...
{
   size_type len = 0;
   API_Global_GetGlobalFont( globalId.c_str(), 0, &len, 0 );
   ...
   if ( len > 0 )
   {
      ...
      if ( API_Global_GetGlobalFont( globalId.c_str(), fontFace.Begin(), &len, &sizeInPoints ) == api_false )
...
{
   size_type len = 0;
   API_Global_GetGlobalString( globalId.c_str(), 0, &len );

   String str;
   if ( len > 0 )
   {
      str.SetLength( len );
      if ( API_Global_GetGlobalString( globalId.c_str(), str.Begin(), &len ) == api_false )
...
PixInsightSettings::variable_type PixInsightSettings::GlobalVariableType( const IsoString& globalId )
{
   ...
   if ( API_Global_GetGlobalFlag( id, 0 ) != api_false )
      return GlobalVariableType::Flag;

   if ( API_Global_GetGlobalInteger( id, 0, api_true ) != api_false )
      return GlobalVariableType::Integer;

   if ( API_Global_GetGlobalInteger( id, 0, api_false ) != api_false )
      return GlobalVariableType::Unsigned;

   if ( API_Global_GetGlobalReal( id, 0 ) != api_false )
      return GlobalVariableType::Real;

   if ( API_Global_GetGlobalString( id, 0, 0 ) != api_false )
      return GlobalVariableType::String;

   if ( API_Global_GetGlobalColor( id, 0 ) != api_false )
      return GlobalVariableType::Color;

   if ( API_Global_GetGlobalFont( id, 0, 0, 0 ) != api_false )
...
{
   if ( API_Global_IsGlobalSettingsUpdateContextActive() )
      throw Error( "Illegal call to PixInsightSettings::BeginUpdate()" );

   if ( !API_Global_EnterGlobalSettingsUpdateContext() )
...
void PixInsightSettings::EndUpdate()
{
   ...
   if ( !API_Global_ExitGlobalSettingsUpdateContext() )
...
void PixInsightSettings::CancelUpdate()
{
   ...
   if ( !API_Global_CancelGlobalSettingsUpdate( ModuleHandle(), 0 ) )
...
{
   if ( !API_Global_SetGlobalFlag( globalId.c_str(), api_bool( b ) ) )
...
{
   if ( !API_Global_SetGlobalInteger( globalId.c_str(), i, api_true ) )
...
{
   if ( !API_Global_SetGlobalInteger( globalId.c_str(), u, api_false ) )
      throw APIFunctionError( "SetGlobalInteger" );
}
...
{
   if ( !API_Global_SetGlobalReal( globalId.c_str(), f ) )
...
{
   if ( !API_Global_SetGlobalColor( globalId.c_str(), c ) )
...
void PixInsightSettings::SetGlobalFont( const IsoString& globalId, const Font& f )
{
   ...
   if ( !API_Global_SetGlobalFont( globalId.c_str(), fontFace.c_str(), RoundInt( f.PointSize() ) ) )
...
{
   if ( !API_Global_SetGlobalString( globalId.c_str(), s.c_str() ) )
