...
CheckBox::CheckBox( const String& text, Control& parent )
   : Button( API_Button_CreateCheckBox( ModuleHandle(), this, text.c_str(), parent.handle, 0/*flags*/ ) )
{
   if ( handle == 0 )
...
{
   return API_Button_GetButtonTristateEnabled( handle ) != api_false;
}
...
{
   API_Button_SetButtonTristateEnabled( handle, tristate );
}
