...
// ----------------------------------------------------------------------------
// pcl/FileFormat.cpp - Released 2025-04-07T08:53:32Z
// ----------------------------------------------------------------------------
...
public:
   ...
   {
      if ( !API_FileFormat_GetFileFormatCapabilities( handle, &capabilities ) )
...
#define formats   reinterpret_cast<Array<FileFormat>*>( data )
      try
      {
         formats->Add( std::move( FileFormat( handle ) ) );
         return api_true;
      }
      catch ( ... )
      {
...
FileFormat::FileFormat( const String& nameExtOrMime, bool toRead, bool toWrite )
{
   ...
   if ( nameExtOrMime.Contains( '/' ) )
   {
      ...
      m_data->handle = API_FileFormat_GetFileFormatByMimeType( ModuleHandle(), mimeType.c_str(), toRead, toWrite );
   ...
   {
      m_data->handle = API_FileFormat_GetFileFormatByFileExtension( ModuleHandle(), nameExtOrMime.c_str(), toRead, toWrite );
      if ( m_data->handle == nullptr )
         throw Error( "FileFormat: No installed image file format was found "
   ...
   else
   {
      ...
      m_data->handle = API_FileFormat_GetFileFormatByName( ModuleHandle(), id.c_str() );
      if ( m_data->handle == nullptr )
         throw Error( "FileFormat: No installed image file format was found "
...
FileFormat::~FileFormat()
{
   ...
   // because FileFormatPrivate is not defined.
}
...
{
   size_type len = 0;
   API_FileFormat_GetFileFormatName( m_data->handle, 0, &len );

   IsoString name;
   if ( len > 0 )
   {
      name.SetLength( len );
      if ( API_FileFormat_GetFileFormatName( m_data->handle, name.Begin(), &len ) == api_false )
...
StringList FileFormat::FileExtensions() const
{
   size_type count = 0;
   size_type maxLen = 0;
   API_FileFormat_GetFileFormatFileExtensions( m_data->handle, 0, &count, &maxLen );
   ...
   if ( count > 0 )
   {
      ...
      if ( API_FileFormat_GetFileFormatFileExtensions( m_data->handle, ptrs.Begin(), &count, &maxLen ) == api_false )
...
IsoStringList FileFormat::MimeTypes() const
{
   size_type count = 0;
   size_type maxLen = 0;
   API_FileFormat_GetFileFormatMimeTypes( m_data->handle, 0, &count, &maxLen );
   ...
   if ( count > 0 )
   {
      ...
      if ( API_FileFormat_GetFileFormatMimeTypes( m_data->handle, ptrs.Begin(), &count, &maxLen ) == api_false )
...
{
   return API_FileFormat_GetFileFormatVersion( m_data->handle );
}
...
{
   size_type len = 0;
   API_FileFormat_GetFileFormatDescription( m_data->handle, 0, &len );

   String description;
   if ( len > 0 )
   {
      description.SetLength( len );
      if ( API_FileFormat_GetFileFormatDescription( m_data->handle, description.Begin(), &len ) == api_false )
...
{
   size_type len = 0;
   API_FileFormat_GetFileFormatImplementation( m_data->handle, 0, &len );

   String implementation;
   if ( len > 0 )
   {
      implementation.SetLength( len );
      if ( API_FileFormat_GetFileFormatImplementation( m_data->handle, implementation.Begin(), &len ) == api_false )
...
{
   size_type len = 0;
   API_FileFormat_GetFileFormatStatus( m_data->handle, 0, &len, 0/*reserved*/ );

   String status;
   if ( len > 0 )
   {
      status.SetLength( len );
      if ( API_FileFormat_GetFileFormatStatus( m_data->handle, status.Begin(), &len, 0/*reserved*/ ) == api_false )
...
{
   return Bitmap( API_FileFormat_GetFileFormatIcon( m_data->handle ) );
}
...
{
   return Bitmap( API_FileFormat_GetFileFormatSmallIcon( m_data->handle ) );
}
...
{
   return API_FileFormat_ValidateFormatSpecificData( m_data->handle, block ) != api_false;
}

void FileFormat::DisposeFormatSpecificData( void* block ) const
{
   API_FileFormat_DisposeFormatSpecificData( m_data->handle, block );
}
...
{
   return API_FileFormat_EditFileFormatPreferences( m_data->handle ) != api_false;
}
...
Array<FileFormat> FileFormat::AllFormats()
{
   ...
   if ( API_FileFormat_EnumerateFileFormats( FileFormatPrivate::EnumerationCallback, &formats ) == api_false )
...
bool FileFormat::IsSupportedFileFormatBySuffix( const String& path, bool toRead, bool toWrite )
{
   ...
   if ( pathOrSuffix.IsEmpty() )
      return false;
   return API_FileFormat_GetFileFormatByFileExtension( ModuleHandle(), pathOrSuffix.c_str(), toRead, toWrite ) != nullptr;
}
                         ...
                         bool toRead, bool toWrite, bool recursive, bool followLinks )
{
   ...
   for ( File::Find f( dirPath + "/*" ); f.NextItem( info ); )
      ...
      if ( followLinks || !info.attributes.IsFlagSet( FileAttribute::SymbolicLink ) )
         ...
         {
            if ( API_FileFormat_GetFileFormatByFileExtension( ModuleHandle(), info.name.c_str(), toRead, toWrite ) != nullptr )
...
StringList FileFormat::EphemerisFiles( const String& dirPath, bool recursive, bool followLinks )
{
   return FindFilesBySuffix( dirPath, StringList() << ".xeph", recursive, followLinks );
}
...
// ----------------------------------------------------------------------------
// EOF pcl/FileFormat.cpp - Released 2025-04-07T08:53:32Z
