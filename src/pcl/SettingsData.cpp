...
// ----------------------------------------------------------------------------
// pcl/SettingsData.cpp - Released 2025-04-07T08:53:32Z
// ----------------------------------------------------------------------------
   ...
   , m_universal( universal )
{
   ...
   int instance = API_Global_ApplicationInstanceSlot( ModuleHandle() );
   if ( instance <= 0 )
      throw Error( "SettingsData: Invalid application instance" );

   size_type len = 0;
   API_Global_GetApplicationConfigurationDirectory( 0, &len );
   if ( len == 0 )
      throw Error( "SettingsData: Internal error: Unable to get the application configuration directory" );

   String confDirPath;
   confDirPath.SetLength( len );
   if ( API_Global_GetApplicationConfigurationDirectory( confDirPath.Begin(), &len ) == api_false )
...
{
   if ( m_universal )
      if ( API_Global_ApplicationInstanceSlot( ModuleHandle() ) != 1 )
...
// ----------------------------------------------------------------------------
// EOF pcl/SettingsData.cpp - Released 2025-04-07T08:53:32Z
