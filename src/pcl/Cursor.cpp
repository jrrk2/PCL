...
Cursor::Cursor( std_cursor stdShape )
   : UIObject( API_Cursor_CreateCursor( ModuleHandle(), stdShape ) )
{
   if ( IsNull() )
...
Cursor::Cursor( const Bitmap& p, const pcl::Point& hotSpot )
   : UIObject( API_Cursor_CreateBitmapCursor( ModuleHandle(), p.handle, hotSpot.x, hotSpot.y ) )
{
   if ( IsNull() )
...
Cursor::Cursor( const Bitmap& p, int hotSpotX, int hotSpotY )
   : UIObject( API_Cursor_CreateBitmapCursor( ModuleHandle(), p.handle, hotSpotX, hotSpotY ) )
{
   if ( IsNull() )
      throw APIFunctionError( "CreateBitmapCursor" );
}
...
{
   pcl::Point p;
   API_Cursor_GetCursorHotSpot( handle, &p.x, &p.y );
   return p;
}
...
{
   pcl::Point p;
   API_Global_GetCursorPosition( &p.x, &p.y );
   return p;
}
...
{
   API_Global_SetCursorPosition( x, y );
}
...
{
   return API_Cursor_CloneCursor( ModuleHandle(), handle );
}
