...
private:
   ...
   void GetProperties()
   {
      ...
      if ( handle != nullptr )
         if ( !API_Process_GetProcessProperties( handle, &properties ) )
...
{
   m_data = new ProcessPrivate( API_Process_GetProcessByName( ModuleHandle(), id.c_str() ) );
...
{
   size_type len = 0;
   API_Process_GetProcessIdentifier( m_data->handle, 0, &len );

   IsoString id;
   if ( len > 0 )
   {
      id.SetLength( len );
      if ( API_Process_GetProcessIdentifier( m_data->handle, id.Begin(), &len ) == api_false )
...
{
   size_type len = 0;
   API_Process_GetProcessAliasIdentifiers( m_data->handle, 0, &len );
   ...
   if ( len > 0 )
   {
      ...
      if ( API_Process_GetProcessAliasIdentifiers( m_data->handle, csAliases.Begin(), &len ) == api_false )
...
{
   size_type len = 0;
   API_Process_GetProcessCategory( m_data->handle, 0, &len );
   ...
   if ( len > 0 )
   {
      ...
      if ( API_Process_GetProcessCategory( m_data->handle, csCategories.Begin(), &len ) == api_false )
...
{
   return API_Process_GetProcessVersion( m_data->handle );
}
...
{
   size_type len = 0;
   API_Process_GetProcessDescription( m_data->handle, 0, &len );

   String description;
   if ( len > 0 )
   {
      description.SetLength( len );
      if ( API_Process_GetProcessDescription( m_data->handle, description.Begin(), &len ) == api_false )
...
{
   size_type len = 0;
   API_Process_GetProcessScriptComment( m_data->handle, 0, &len );

   String comment;
   if ( len > 0 )
   {
      comment.SetLength( len );
      if ( API_Process_GetProcessScriptComment( m_data->handle, comment.Begin(), &len ) == api_false )
...
{
   return Bitmap( API_Process_GetProcessIcon( m_data->handle ) );
}
...
{
   return Bitmap( API_Process_GetProcessSmallIcon( m_data->handle ) );
}
...
{
   return API_Process_EditProcessPreferences( m_data->handle ) != api_false;
}
...
{
   return API_Process_BrowseProcessDocumentation( m_data->handle, 0/*flags*/ ) != api_false;
}
...
{
   return int( API_Process_RunProcessCommandLine( m_data->handle, arguments.c_str() ) );
}
...
{
   return API_Process_LaunchProcess( m_data->handle ) != api_false;
}
...
Process::parameter_list Process::Parameters() const
{
   ...
   if ( API_Process_EnumerateProcessParameters( m_data->handle,
...
{
   return API_Process_GetParameterByName( m_data->handle, paramId.c_str() ) != 0;
}

bool Process::HasTableColumn( const IsoString& tableId, const IsoString& colId ) const
{
   meta_parameter_handle hTable = API_Process_GetParameterByName( m_data->handle, tableId.c_str() );
   return hTable != 0 && API_Process_GetTableColumnByName( m_data->handle, colId.c_str() ) != 0;
}
...
{
   size_type len = 0;
   API_Process_EnumerateProcessCategories( 0, 0, &len, 0 );
   ...
   if ( len > 0 )
   {
      ...
      if ( API_Process_EnumerateProcessCategories( CategoryEnumerationCallback,
...
Array<Process> Process::AllProcesses()
{
   ...
   if ( API_Process_EnumerateProcesses( InternalProcessEnumerator::ProcessCallback, &processes ) == api_false )
...
Array<Process> Process::ProcessesByCategory( const IsoString& category )
{
   ...
   if ( API_Process_EnumerateProcesses( InternalProcessEnumerator::CategoryCallback, &enumeration ) == api_false )
      throw APIFunctionError( "EnumerateProcesses" );
